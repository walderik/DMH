...
class IntrigueType extends SelectionData{
    ...
    public static function getIntrigeTypesForLarpAndRole($larpId, $roleId) {
        ...
        $sql = "SELECT * from `intriguetype` WHERE Id in (SELECT * FROM `intriguetype_larp_role` WHERE LARP_RoleLARPid = ? AND LARP_RoleRoleId = ?)  ORDER BY ORDER BY SortOrder;";
    ...
    public static function getIntrigeTypesForLarpAndGroup($larpId, $groupId) {
        ...
        $sql = "SELECT * from `intriguetype` WHERE Id in (SELECT * FROM `intriguetype_larp_group` WHERE LARP_GroupGroupId = ? AND LARP_GroupLARPId = ?) ORDER BY ORDER BY SortOrder;";
        $stmt = static::connectStatic()->prepare($sql);
