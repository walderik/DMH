...
if (isset($_REQUEST["personId"])) $personId = $_REQUEST["personId"];
...
if (isset($personId)) {
    ...
    if (isset($fromGroupId)) {
        $group = Group::loadById($fromGroupId);
        if (isset($fromHouseId)) {
            $fromHouse = House::loadById($fromHouseId);
            $personsInFromGroup = Person::getGroupMembersInHouse($group, $fromHouse, $current_larp);
            $res[0] = count($personsInFromGroup);
            
        } else {
            $personsInFromGroup = Person::getPersonsInGroupWithoutHousing($group, $current_larp);
            $res[0] = count($personsInFromGroup);
        }
    }
    if (isset($fromHouseId)) {
